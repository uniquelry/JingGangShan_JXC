...
import com.uniquelry.jgs.entity.SaleCount;
...
public class SaleListAdminController {
	...
	public String genBillCode(String type)throws Exception{
		...
		if(saleNumber!=null){
			biilCodeStr.append(StringUtils.formatCode(saleNumber));
		}else{
	...
	public Map<String,Object> countSaleByDay(String begin,String end)throws Exception{
		...
		List<String> datas= DateUtils.getRangeDates(begin, end);
		...
		for(String data:datas){
			...
			for(Object o:ll){
				...
				if(dd.equals(data)){ // 存在
					scd.setAmountCost(MathUtils.format2Bit(Float.parseFloat(oo[0].toString())));
					scd.setAmountSale(MathUtils.format2Bit(Float.parseFloat(oo[1].toString())));
					scd.setAmountProfit(MathUtils.format2Bit(scd.getAmountSale()-scd.getAmountCost()));
					flag=false;
				}
			}
			if(flag){
	...
	public Map<String,Object> countSaleByMonth(String begin,String end)throws Exception{
		...
		List<String> datas= DateUtils.getRangeMonth(begin, end);
